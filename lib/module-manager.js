...
const $service = require('./service/utils/service');
...
module.exports = class {
    ...
    async install(repository) {
        ...
        await Git.Clone(`https://github.com/${ repository }`, modulePath);
        await $utils.exec('npm install', {
            cwd: modulePath
        });

        await self.load(modulePath);
    }
    ...
    loadSystemPackages() {
        var self = this;
        return $lodash.reduce(this._modules, (promise, module, name) => {
    ...
    loadSystemServices() {
        var self = this;
        return $q.all($lodash.map(this._modules, (module, name) => {
